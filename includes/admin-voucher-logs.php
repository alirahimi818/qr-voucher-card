...
add_action('admin_menu', 'add_qr_voucher_log_menu_to_admin');

function add_qr_voucher_log_menu_to_admin()
{
    add_submenu_page('qr-voucher-card', __('Voucher usage histories', 'qrvc'), __('Voucher histories', 'qrvc'), 'manage_options', 'qr-voucher-logs', 'qr_voucher_log_admin_page', 'dashicons-tickets-alt');
}

function qr_voucher_log_admin_page()
{
    ...
    global $wpdb;
    $vouchers_table_name = $wpdb->prefix . "qr_vouchers";
    $voucher_logs_table_name = $wpdb->prefix . "qr_voucher_logs";
    ...
    $query = "FROM {$voucher_logs_table_name} INNER JOIN {$vouchers_table_name} ON {$voucher_logs_table_name}.voucher_id={$vouchers_table_name}.id ";

    if (@$_GET['s']) {
        $s = $_GET['s'];
        $query .= "WHERE {$vouchers_table_name}.code LIKE '%{$s}%' ";
    }
    ...
    $query = qr_where_between_date_query($query, "{$voucher_logs_table_name}.created_at", $from_date, $to_date);
    $count_query = "SELECT COUNT(*) " . $query;
    $query = "SELECT {$voucher_logs_table_name}.*, {$vouchers_table_name}.code, {$vouchers_table_name}.price as voucher_price " . $query;
    ...
    ?>
    <div class="wrap">
        <h1 class="wp-heading-inline"><?php _e('Voucher usage histories', 'qrvc') ?></h1>
        <form action="" method="GET" class="qr-search-form">
            <input type="hidden" name="page" value="qr-voucher-logs">
            <p class="search-box" style="margin-bottom: 10px;">
                <input type="text" id="search-input" name="s" value="<?php echo @$_GET['s']; ?>"
                       placeholder="<?php _e('Search', 'qrvc') ?>...">
        ...
        <div><?php echo @$_GET['s'] || @$_GET['from_date'] || @$_GET['to_date'] ? __('items count') . ': '. $items_count : '' ?></div>
            ...
            <?php if (!empty($items)) {
                foreach ($items as $item) { ?>
                    <tr>
                        ...
                        <td><?php echo $price_symbol . $item->voucher_price ?></td>
                        <td class="red-color"><?php echo $price_symbol . $item->price ?></td>
                ...
                ?>
                <tr>
                    <td class="text-center red" colspan="4"><?php _e('Not found!', 'qrvc') ?></td>
                </tr>
                <?php
            }
            ?>
